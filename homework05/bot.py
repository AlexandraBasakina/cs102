...
telebot.apihelper.proxy = {'https': 'https://118.27.28.182:3128'}
...
def get_day(message: str) -> None:
    try:
        ...
        if days[day] == 7:
            bot.send_message(message.chat.id, 'Спите, сударь(-ыня), это выходной')
            return None
    except IndexError:
        bot.send_message(message.chat.id,
                         'Не хватает параметров, судрь(-ыня)')
    resp = ''
...
def get_near_lesson(message: str) -> None:
    ...
    try:
        ...
        bot.send_message(message.chat.id,
                         'Не хватает параметров, сударь(-ыня)')
...
@bot.message_handler(commands=['WhatAbout'])
def about(message: str) -> None:
    message = str(message)
    message = message.split()
    surname = message[1]
    if surname == 'Sorokin' or surname == 'Сорокин':
        bot.send_message(message.chat.id, 'Сорокин - это один из лучших учителей программирования. Но лучше только тот, кто его учил.')
    elif surname == 'Pelevin' or surname == 'Пелеви':
        bot.send_message(message.chat.id, 'Пелевин - это лапочка, которого мы все любим.')
    elif surname == 'Andrew' or surname == 'Андрей':
        bot.send_message(message.chat.id, 'Андрей - это самый лучший магистрант. Он пендантичин и в некоторм роде мил')
    elif surname == 'Orel' or surname == 'Орёл':
        bot.send_message(message.chat.id, 'Орёл ДаниИл - это САМЫЙ лучший одногруппник. Этакое маленькое солнышко знаний среди хмурых тучек)')
